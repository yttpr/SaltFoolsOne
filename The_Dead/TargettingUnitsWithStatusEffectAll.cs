...
// Type: THE_DEAD.TargettingUnitsWithStatusEffectAll
  ...
  public class TargettingUnitsWithStatusEffectAll : BaseCombatTargettingSO
  {
      ...
      bool isCasterCharacter)
    {
      ...
      foreach (CombatSlot characterSlot in slots.CharacterSlots)
      {
        ...
        if (targetSlotInformation != null && targetSlotInformation.HasUnit && !TargettingUnitsWithStatusEffectAll.IsUnitAlreadyContained(targets, targetSlotInformation) && !this.IsCastSlot(casterSlotID, targetSlotInformation) && targetSlotInformation.Unit.ContainsStatusEffect(this.targetStatus))
