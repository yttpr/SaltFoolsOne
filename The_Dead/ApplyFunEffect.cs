...
namespace THE_DEAD
{
  public class ApplyFunEffect : EffectSO
  {
    [SerializeField]
      ...
      out int exitAmount)
    {
      ...
      stats.statusEffectDataBase.TryGetValue((StatusEffectType) 422442, out effectInfo);
      for (int index1 = 0; index1 < targets.Length; ++index1)
      {
        if (targets[index1].HasUnit)
        {
          ...
          Fun_StatusEffect funStatusEffect = new Fun_StatusEffect(amount);
          funStatusEffect.SetEffectInformation(effectInfo);
          IStatusEffector unit = targets[index1].Unit as IStatusEffector;
          bool flag = false;
          int index2 = 999;
          for (int index3 = 0; index3 < unit.StatusEffects.Count; ++index3)
          {
            if (unit.StatusEffects[index3].EffectType == funStatusEffect.EffectType)
            {
              index2 = index3;
              flag = true;
            }
          ...
          else if (targets[index1].Unit.ApplyStatusEffect((IStatusEffect) funStatusEffect, amount))
            ++exitAmount;
        }
      }
      return exitAmount > 0;
