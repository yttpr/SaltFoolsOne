...
// Type: THE_DEAD.PigmentDamageEffect
...
namespace THE_DEAD
{
  public class PigmentDamageEffect : EffectSO
  {
    [SerializeField]
      ...
      out int exitAmount)
    {
      ...
      foreach (TargetSlotInfo target in targets)
      {
        if (target.HasUnit)
        {
          ...
          if (this._indirect)
          {
            damageInfo = target.Unit.Damage(amount1, (IUnit) null, this._deathType, targetSlotOffset, false, false, true, DamageType.Mana);
          }
          else
          {
            int amount2 = caster.WillApplyDamage(amount1, target.Unit);
            damageInfo = target.Unit.Damage(amount2, caster, this._deathType, targetSlotOffset, ignoresShield: this._ignoreShield, specialDamage: DamageType.Mana);
          }
