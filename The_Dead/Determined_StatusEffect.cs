...
// Type: THE_DEAD.Determined_StatusEffect
...
namespace THE_DEAD
{
  public class Determined_StatusEffect : IStatusEffect, ITriggerEffect<IStatusEffector>
  {
    ...
    {
      CombatManager.Instance.AddObserver(new Action<object, object>(this.OnStatusTriggered), TriggerCalls.CanDie.ToString(), (object) caller);
      CombatManager.Instance.AddObserver(new Action<object, object>(this.OnTurnEnd), TriggerCalls.OnTurnStart.ToString(), (object) caller);
    }

    public void OnTriggerDettached(IStatusEffector caller)
    {
      CombatManager.Instance.RemoveObserver(new Action<object, object>(this.OnStatusTriggered), TriggerCalls.CanDie.ToString(), (object) caller);
      CombatManager.Instance.RemoveObserver(new Action<object, object>(this.OnTurnEnd), TriggerCalls.OnTurnStart.ToString(), (object) caller);
    }

    public void OnSubActionTrigger(object sender, object args, bool stateCheck)
    {
      int amount = this.Amount;
      if ((sender as IUnit).ContainsStatusEffect(StatusEffectType.Cursed) || (sender as IUnit).ContainsPassiveAbility(PassiveAbilityTypes.Dying) || (sender as IUnit).ContainsPassiveAbility(PassiveAbilityTypes.Dying))
        (sender as IUnit).DirectDeath((IUnit) null);
      else
        (sender as IUnit).Heal(amount, HealType.Heal, true);
