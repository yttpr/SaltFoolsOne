...
// Type: THE_DEAD.ApplyAnestheticsRangePlusTwoEffect
...
namespace THE_DEAD
{
  public class ApplyAnestheticsRangePlusTwoEffect : EffectSO
  {
    [SerializeField]
      ...
      out int exitAmount)
    {
      ...
      entryVariable = UnityEngine.Random.Range(entryVariable, entryVariable + 3);
      for (int index1 = 0; index1 < targets.Length; ++index1)
      {
        if (targets[index1].HasUnit)
        {
          ...
          IStatusEffect statusEffect = (IStatusEffect) new Anesthetics_StatusEffect(amount);
          ...
          if (flag)
          {
            foreach (MethodBase constructor in unit.StatusEffects[index2].GetType().GetConstructors())
            {
              ...
              {
                IStatusEffect instance = (IStatusEffect) Activator.CreateInstance(unit.StatusEffects[index2].GetType(), (object) amount, (object) 0);
              }
            }
          }
          else if (targets[index1].Unit.ApplyStatusEffect(statusEffect, amount))
            ++exitAmount;
        }
      }
      return exitAmount > 0;
