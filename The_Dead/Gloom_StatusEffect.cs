...
// Type: THE_DEAD.Gloom_StatusEffect
...
namespace THE_DEAD
{
  public class Gloom_StatusEffect : IStatusEffect, ITriggerEffect<IStatusEffector>
  {
    ...
    public void OnTriggerAttached(IStatusEffector caller)
    {
      ...
      CombatManager.Instance.AddObserver(new Action<object, object>(this.OnTurnEnd), TriggerCalls.OnTurnFinished.ToString(), (object) caller);
    }
    ...
    public void OnTriggerDettached(IStatusEffector caller)
    {
      ...
      CombatManager.Instance.RemoveObserver(new Action<object, object>(this.OnTurnEnd), TriggerCalls.OnTurnFinished.ToString(), (object) caller);
    }
    ...
    public void OnSubActionTrigger(object sender, object args, bool stateCheck)
    {
      if (!(sender as IUnit).ContainsStatusEffect(StatusEffectType.Cursed))
        return;
      int amount = UnityEngine.Random.Range(7, 14);
      (sender as IUnit).Damage(amount, (IUnit) null, DeathType.Basic, -1, false, false, true);
    }
    ...
    public void OnStatusTriggered(object sender, object args)
    {
      ...
      this.IncreaseDuration(sender as IStatusEffector, amount);
      if (this.Amount <= 6)
        return;
    ...
    public void IncreaseDuration(IStatusEffector effector, int amount)
    {
      ...
      this.Amount += amount;
      if (amount1 == this.Amount)
        return;
      effector.StatusEffectValuesChanged(this.EffectType, this.Amount - amount1);
    }
